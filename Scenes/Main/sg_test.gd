...
@onready var character : SGCharacterBody2D = $Character
...
func _physics_process(delta: float) -> void:
	...
	cube.rotation.z = -character.rotation
	cube.position = Vector3(character.position.x, -character.position.y, 0) / 100
	print(cube.position)
